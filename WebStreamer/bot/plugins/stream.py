...
async def media_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        await m.reply_text(
            text=msg_text.format(get_name(m), humanbytes(get_media_file_size(m))),
            quote=True,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
...
@StreamBot.on_message(filters.command(["link"]) & filters.chat(Var.AUTH_CHAT))
async def group_media_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        if msg == None or not msg.video:
            return await m.reply_text("Reply To a Valid Video")
    except Exception as e:
        return await m.reply_text(f"Error: {e}")
    try:
        log_msg = await msg.forward(chat_id=Var.BIN_CHANNEL)
        file_hash = get_hash(log_msg, Var.HASH_LENGTH)
        stream_link = f"{Var.URL}{log_msg.id}/{quote_plus(get_name(msg))}?hash={file_hash}"
        short_link = f"{Var.URL}{file_hash}{log_msg.id}"
        logger.info(f"Generated link: {stream_link} for {m.from_user.first_name}")
        ...
        await m.reply_text(
            text=msg_text.format(get_name(msg), humanbytes(get_media_file_size(msg))),
            quote=True,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
